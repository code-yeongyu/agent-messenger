...
export async function getAction(channelId, messageId, options) {
    try {
        const credManager = new DiscordCredentialManager();
        const config = await credManager.load();
        if (!config.token) {
...
export async function deleteAction(channelId, messageId, options) {
    try {
        const credManager = new DiscordCredentialManager();
        const config = await credManager.load();
        if (!config.token) {
...
export async function ackAction(channelId, messageId, options) {
    try {
        const credManager = new DiscordCredentialManager();
        const config = await credManager.load();
        if (!config.token) {
...
export async function searchAction(query, options) {
    try {
        ...
        const searchOptions = {};
        if (options.channel)
            searchOptions.channelId = options.channel;
        if (options.author)
            searchOptions.authorId = options.author;
        if (options.has) {
            searchOptions.has = options.has;
        }
        if (options.sort) {
            searchOptions.sortBy = options.sort;
        }
        if (options.sortDir) {
            searchOptions.sortOrder = options.sortDir;
        }
