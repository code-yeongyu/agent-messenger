...
export class TokenExtractor {
    ...
    slackDir;
    keyCache;
    constructor(platform, slackDir, keyCache) {
    ...
    async extractTokensFromLevelDB() {
        const possibleDirs = [
    ...
    async extractFromLevelDB(dbPath) {
        const tokens = [];
    ...
    extractTokensFromLDBFiles(dbPath) {
        const tokens = [];
        try {
    ...
    extractTokenFromLogFile(buffer, startIdx) {
    ...
    extractTokenFromBuffer(buffer, startIdx) {
    ...
    getDerivedKeyFromKeychain() {
        ...
        try {
            let password;
            try {
                password = execSync('security find-generic-password -ga "Slack App Store Key" -s "Slack Safe Storage" -w 2>/dev/null', { encoding: 'utf8' }).trim();
            }
            catch {
                password = execSync('security find-generic-password -ga "Slack Key" -s "Slack Safe Storage" -w 2>/dev/null', { encoding: 'utf8' }).trim();
            }
