...
Each `KakaoMember` includes `user_id`, `nickname`, profile image URLs, `status_message`, `country_iso`, `user_type`, and open-chat-only fields (`open_token`, `open_profile_link_id`, `open_permission`).
