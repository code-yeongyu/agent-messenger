...
### Expressions (Emoji and Stickers)

```typescript
// List a guild's custom emoji
const emojis = await client.listEmojis(guildId)
// → DiscordEmoji[] { id, name, animated?, managed?, available?, roles?, user? }

// Upload a custom emoji. The media type is read from the bytes, not the filename.
const bytes = new Uint8Array(await readFile('./potato.png'))
const emoji = await client.createEmoji(guildId, 'potato', bytes, 'potato.png')

await client.deleteEmoji(guildId, emoji.id)

// List a guild's custom stickers
const stickers = await client.listStickers(guildId)
// → DiscordSticker[] { id, name, tags, type, format_type, description?, ... }
// type: STANDARD = 1, GUILD = 2. format_type: PNG = 1, APNG = 2, LOTTIE = 3, GIF = 4.

// Upload a custom sticker. `tags` is the unicode emoji it relates to.
const sticker = await client.createSticker(
  guildId,
  { name: 'potato', description: 'A potato', tags: '🥔' },
  bytes,
  'potato.png',
)
...
These methods take image bytes rather than a path, so the caller decides how the
image is read. The media type the upload declares is read from the bytes
themselves — a filename is unverified input, and Discord refuses a part whose
declared type does not match its content with a bare `Invalid Asset`.

Each endpoint takes its own formats: emoji accepts PNG, GIF, JPEG and WebP;
stickers accept PNG (including APNG), GIF and a JSON document for Lottie.
Bytes that are not one of the target endpoint's formats throw a `DiscordError`
with code `unsupported_asset` before any request is made. A sticker's JSON must
carry the `layers` array every Lottie animation has — `{}` is otherwise sent and
refused as `Invalid Asset`. Whether the animation renders is Discord's call, and
Lottie uploads need a `VERIFIED` or `PARTNERED` guild.

Listing needs no special permission. Creating needs `CREATE_GUILD_EXPRESSIONS`.
Deleting an expression you created needs either `CREATE_GUILD_EXPRESSIONS` or
`MANAGE_GUILD_EXPRESSIONS`; deleting someone else's needs `MANAGE_GUILD_EXPRESSIONS`. `getServer(guildId)`
returns `premium_tier` with the guild's `emojis` and `stickers`, which is how
you find out how many slots remain.
...
// Get only unread DM and group-DM channels, newest first
const unreadDMs = await client.getUnreadDMs()
const capped = await client.getUnreadDMs({ limit: 10 })
// → DiscordUnreadDMsResult { channels, count, totalUnread, complete }
```

Mention history only records guild mentions, so a DM never appears in `getUnreadMentions` regardless of how many unread messages it holds. `getUnreadDMs` covers that gap by comparing each DM channel's `last_message_id` against its read marker. Each channel's `unreadCount` is Discord's own badge counter, which in DMs counts every message rather than only mentions: it is `null` when the channel has no read-state entry (count unknown, but still reported as unread, since a DM holding messages with no marker is more likely never opened than read) and `0` when the DM is muted but genuinely unread. `count` and `totalUnread` describe every unread DM found, so they stay accurate when `limit` caps the returned list; `totalUnread` sums known counts only. `complete` is `false` when Discord delivered a partial read state: an absent entry only means "never opened" when the collection is complete, so in a partial payload those channels are omitted rather than reported, and the list may be short.
...
// Same snapshot, plus whether Discord delivered a complete collection
const snapshot = await client.fetchReadStateSnapshot()
// → DiscordReadStateSnapshot { states, partial }
```
...
// Get your own membership (including role IDs) in a server
const member = await client.getMyGuildMember(serverId)
// → DiscordGuildMember { user, nick?, roles, joined_at, deaf, mute, flags }

// List a server's role definitions
const roles = await client.listRoles(serverId)
// → DiscordRole[] { id, name, color, hoist, position, permissions, managed, mentionable }
